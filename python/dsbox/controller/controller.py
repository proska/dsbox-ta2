...
class Controller:
    ...
    def initialize_from_ta3(self, config: typing.Dict):
        ...
        self._logger.info('problem data resource types: {}'.format(self.taskSourceType))
    ...
    def train(self) -> Status:
        ...
        for count, idx in enumerate(self.select_next_template(max_iter=self.max_iter)):
            # if count > 2:
            #     break
            template = self.template[idx]
            self._logger.info(STYLE+"Template {}:{} Selected. UCT:{}".format(idx, template.template['name'], self.uct_score))
